...
/*package*/ abstract class BaseParams {
	...
	private volatile boolean multipart = false; // 是否强制使用multipart表单
	private volatile boolean asJsonContent = false; // 用json形式的bodyParams上传
	...
	private void checkBodyParams() {
		...
		if (!HttpMethod.permitsRequestBody(method) || !TextUtils.isEmpty(bodyContent) || requestBody != null) {
